...
`gmm_clustering.py`(엔트리)는 가능한 얇게 유지하고,
실제 파이프라인 실행/아티팩트/리포트/시각화 연결은 여기서 담당합니다.
"""
...
from src.gmm.data_loader import FEATURE_COLUMNS
...
def run_gmm_pipeline(
    *,
    ...
    # 명시적으로 지정한 FEATURE_COLUMNS만 사용합니다.
    # (자동 후보 확장/결측률 필터/자동 PCA 등은 사용하지 않음)
    ...
    (
        ...
        use_pca=False,
        apply_iforest=True,
        # "걸러내는 로직" 비활성화: 저분산/고상관 제거를 사실상 수행하지 않음
    ...
    # CSV용: 구성 종목(중복 제거된 전체 리스트)
    members_map = build_cluster_members(df_valid)
    # 리포트용: 상태에 자주 진입한 종목 TOP
